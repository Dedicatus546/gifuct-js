...
import { SchemaFactory } from 'js-binary-schema-parser/lib/schemas/gif'
import { parse } from 'js-binary-schema-parser'
import * as Parser from 'js-binary-schema-parser/lib/parsers/uint8'
import * as ChunkedParser from 'js-binary-schema-parser/lib/parsers/chunked-uint8'
...
export const parseGIF = (arrayBuffer, events = {}) => {
  const byteData = new Uint8Array(arrayBuffer)
  const result = {};
  return parse(Parser.buildStream(byteData), GIF, result, result, events);
}

export const parseGIFChunked = (stream, parser, events = {}) => {
  const result = {};
  const GIF = SchemaFactory(parser);
